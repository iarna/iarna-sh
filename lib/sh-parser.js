...
class ShError extends Error {
  constructor (msg) {
    super(msg)
    this.name = 'ShError'
    /* istanbul ignore next */
    if (Error.captureStackTrace) Error.captureStackTrace(this, ShError)
    this.fromTOML = true
    this.wrapped = null
  }
}
/*
ShError.wrap = err => {
  const terr = new ShError(err.message)
  terr.code = err.code
  terr.wrapped = err
  return terr
}
...
//    }
    ...
    parseListNext (cmd) {
      if (cmd.value.length === 0) throw this.error(new ShError(`Unexpected empty command`))
...
//        return this.return()
    ...
    parseListMore () {
      if (this.char === Parser.END || this.char === CHAR_RP) {
        throw this.error(new ShError('incomplete expression'))
      } else {
        return this.goto(this.parseList)
      }
    }
